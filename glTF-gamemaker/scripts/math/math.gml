function vec3(_x=0, _y=0, _z=0) : vec4(_x, _y, _z) constructor { }
 ...
 */
function vec4(_x=0, _y=0, _z=0, _w=1) constructor {
	x = _x;
	y = _y;
	z = _z;
	w = _w;
...
function aabb(_v1, _v2) constructor {
	v1 = _v1;
	v2 = _v2;
	...
	static expand = function(_v1, _v2) {
		v1 = vecComponentMin(v1, _v1);
		v1 = vecComponentMin(v1, _v2);
		v2 = vecComponentMax(v2, _v2);
		v2 = vecComponentMax(v2, _v1);
		return self;
	};
...
function vecAddSeries(vecs = [ ]) {
	gml_pragma("forceinline");
	var result = new vec4(0, 0, 0, 0);
...
function matRead(m, row, col) {
	gml_pragma("forceinline");
	return m[row + col * 4];
}

/// write to a matrix at [row,col]
function matWrite(m, row, col, value) {
	gml_pragma("forceinline");
	m[row + col * 4] = value;
}

/// return a column of a matrix as a vector
function matReadColumn(m, col) {
	gml_pragma("forceinline");
	return new vec4(matRead(m, 0, col), matRead(m, 1, col), matRead(m, 2, col), matRead(m, 3, col));
}

/// write a column of a matrix with a vector, doing nothing if the vector is undefined
function matWriteColumn(m, col, vec=undefined) {
	gml_pragma("forceinline");
...
function matBuildFromVectors(vecs=[ ]) {
	gml_pragma("forceinline");
	var n = array_length(vecs);
	var result = matrix_build_identity();
	for (var i = 0; i < 4; i++) {
		if (n > i) {
			matWriteColumn(result, i, vecs[i]);
		}
	}
	return result;
}
...
function mulMats(m1, m2) {
	...
	*/
	...
	//debugPrint(string(matBuildFromVectors(columns)));
	//debugPrint(string(result));
...
function pDist(x0, y0, x1=0, y1=0) {
...
/// for animation
function cubicSplineInterpolate(ak, vk, bk, t) {
	/*
